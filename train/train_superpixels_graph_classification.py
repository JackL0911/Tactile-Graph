...
def train_epoch(model, optimizer, device, data_loader, epoch, drop_percent, temp=0.5, aug_type='nn', head=False):
    ...
    for iter, (batch_graphs, batch_labels, batch_snorm_n, batch_snorm_e) in enumerate(data_loader):
        ...
        aug_list1, aug_list2 = aug.aug_double(aug_batch_graphs, aug_type) # 变为两个图（进行图数据增强）
        batch_graphs, batch_snorm_n, batch_snorm_e= aug.collate_batched_graph(aug_list1) # 拆成图，节点和边
        ...
        # batch_graphs -> batch_graphs, batch_x, batch_e
